    ...
    <div class="container-messages">
        ...
        <div class="sidebar">
            <div class="sidebar-content">
                <div class="p-2">
                    <input id="conversation-search" type="search" class="form-control"
                        placeholder="Search conversations or users..." aria-label="Search conversations">
                </div>
                <?php
                ...
                foreach ($activeConvs as $conv):
                    ...
                    ?>
                    <div class="conversation <?= $unreadCount > 0 ? 'has-unread' : '' ?>" tabindex="0"
                        data-user-id="<?= htmlspecialchars($partnerId) ?>"
                        data-conversation-id="<?= htmlspecialchars($conv['id'] ?? 0) ?>" onclick="startConversation(this)">
                        <div class="conversation-avatar">
                            <img src="<?= !empty($profilePic) ? 'data:' . htmlspecialchars($mimeType) . ';base64,' . base64_encode($profilePic) : '/get_image.php?id=' . htmlspecialchars($partnerId) ?>"
                                ...
                                alt="<?= htmlspecialchars($partnerName) ?>'s profile picture" loading="lazy">
                ...
                <?php if (!empty($potentialConvs)): ?>
                    ...
                    <?php foreach ($potentialConvs as $conv):
                        ...
                        $mimeType = $conv['mime_type'] ?? 'image/png';
                        ?>
                        <div class="conversation" tabindex="0" data-user-id="<?= htmlspecialchars($partnerId) ?>"
        ...
        <div class="chat-section">
            <?php if ($activeConversation): ?>
                ...
                <div id="messages" class="message-list" data-last-id="<?= $initialLastId ?>"
                    data-conversation-id="<?= $conversationId ?>" data-logged-in-user="<?= (int) $loggedInUserId ?>"
                        ...
                        <div class="message <?= $msg['sender_id'] == $loggedInUserId ? 'sent' : 'received' ?>"
                            ...
                            <div class="meta">
                                ...
                                <small class="message-time"
                                    data-created-at="<?= htmlspecialchars($msg['created_at']) ?>"><?php echo htmlspecialchars(date('g:ia', strtotime($msg['created_at']))); ?></small>
                            </div>
